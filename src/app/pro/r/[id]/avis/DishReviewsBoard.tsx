...
const isNegative = (r: ProReview) => r.verdict === 'disappointing' || (r.rating != null && r.rating <= 2);
const isPositive = (r: ProReview) => r.verdict === 'must_return' || (r.rating != null && r.rating >= 4);
const matchFilter = (r: ProReview, f: ListFilter) =>
  f === 'unanswered' ? !r.reply : f === 'negative' ? isNegative(r) : f === 'positive' ? isPositive(r) : true;
...
export default function DishReviewsBoard({
  ...
  const [view, setView] = useState<ViewMode>(
    initialView === 'list' || (initialFilter && initialFilter !== 'all') ? 'list' : 'dish'
  );
  ...
  const [sort, setSort] = useState<SortMode>(initialFilter === 'negative' ? 'rating_asc' : 'recent');
  ...
  const syncUrl = (v: ViewMode, f: ListFilter) => {
    ...
    window.history.replaceState(null, '', slug ? `${pathname}?avis=${slug}` : pathname);
  };
  ...
  const listFiltered = useMemo(() => {
    ...
    const arr = reviews.filter(
      (r) =>
        ...
        (!q || norm(r.dishName ?? '').includes(q) || norm(r.comment ?? '').includes(q))
    );
  ...
  const segBtn = (active: boolean) =>
    `flex-1 rounded-lg px-3 py-1.5 text-sm font-semibold transition-colors ${
      active ? 'bg-[var(--surface)] text-[var(--text)] shadow-sm' : 'text-[var(--text2)] hover:text-[var(--text)]'
    }`;
  ...
  return (
    <>
      {/* Notification « avis en attente » — grosse, en tête. Cliquable : bascule
          en liste filtrée « à répondre » (accès rapide). */}
      {pendingCount > 0 ? (
        <button
          type="button"
          onClick={goPending}
          className="flex w-full items-center gap-3.5 rounded-2xl border border-[var(--primary)]/30 bg-[var(--primary-container)] px-4 py-3.5 text-left transition-colors hover:border-[var(--primary)]"
        >
          <span className="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[var(--primary)] text-lg font-extrabold text-white">
            {pendingCount > 99 ? '99+' : pendingCount}
            <span className="absolute -right-0.5 -top-0.5 h-3 w-3 rounded-full bg-red-500 ring-2 ring-[var(--primary-container)]" />
          </span>
          <span className="min-w-0 flex-1">
            <span className="block text-lg font-extrabold leading-tight text-[var(--text)]">
              {pendingCount} avis en attente
            </span>
            <span className="block text-sm text-[var(--text2)]">
              {pendingCount > 1 ? 'Des clients attendent votre réponse.' : 'Un client attend votre réponse.'}
            </span>
          </span>
          <span aria-hidden className="shrink-0 text-sm font-bold text-[var(--primary)]">Voir →</span>
        </button>
      ) : (
        <div className="flex items-center gap-3 rounded-2xl border border-[var(--accent-success)]/30 bg-[var(--accent-success)]/[0.08] px-4 py-3 text-sm font-semibold text-[var(--accent-success)]">
          <span aria-hidden className="text-base">✓</span> Tous vos avis ont une réponse.
        </div>
      )}

      {/* Bascule du style d'affichage */}
      <div className="mt-4 flex gap-1 rounded-xl border border-[var(--border2)] bg-[var(--bg)] p-1">
        <button type="button" onClick={() => changeView('dish')} aria-pressed={view === 'dish'} className={segBtn(view === 'dish')}>
          Par plat
        </button>
        <button type="button" onClick={() => changeView('list')} aria-pressed={view === 'list'} className={segBtn(view === 'list')}>
          Liste
        </button>
      </div>
      ...
      <div className="relative mt-3">
        <span aria-hidden className="pointer-events-none absolute left-3.5 top-1/2 -translate-y-1/2 text-[var(--text3)]">⌕</span>
        <input
          type="text"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
          placeholder={view === 'dish' ? 'Rechercher un plat…' : 'Rechercher un plat ou un mot…'}
      ...
      {view === 'dish' ? (
            ...
            return (
              ...
              >
                {thumb ? (
                  ...
                  <img src={getThumbnailUrl(thumb)} alt="" className="h-16 w-16 shrink-0 rounded-xl object-cover" />
                ) : (
        ...
        <>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <button type="button" onClick={() => changeFilter('all')} aria-pressed={filter === 'all'} className={chip(filter === 'all')}>
              Tous · {counts.all}
            </button>
            <button type="button" onClick={() => changeFilter('unanswered')} aria-pressed={filter === 'unanswered'} className={chip(filter === 'unanswered')}>
              À répondre · {counts.unanswered}
            </button>
            <button type="button" onClick={() => changeFilter('negative')} aria-pressed={filter === 'negative'} className={chip(filter === 'negative', 'neg')}>
              👎 Négatifs · {counts.negative}
            </button>
            <button type="button" onClick={() => changeFilter('positive')} aria-pressed={filter === 'positive'} className={chip(filter === 'positive', 'pos')}>
          ...
          <div className="mt-2 space-y-3">
            {listFiltered.length > 0 ? (
              ...
              <p className="py-8 text-center text-sm text-[var(--text3)]">Aucun avis pour ce filtre.</p>
            )}
          </div>
        </>
      )}
